...
int main() {
	...
	if (ft_str_is_uppercase(upper) != 1) {
		printf(TAB RED "Doesn't handle string that contains only uppercase\n" RESET);
		return 1;
	}
	printf(TAB GREEN "Handles string that contains only uppercase\n" RESET);

	char alpha[] = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz";
	if (ft_str_is_uppercase(alpha) != 0) {
	...
	strcat(non_alpha, "Ø");
	if (ft_str_is_uppercase(non_alpha) != 0) {
